...
Score each file against skill-creation's requirements:
...
- No restated shared principles. The canonical block is quoted in skill-creation; a paraphrase of any of its three numbered items counts as a restatement. Domain-specific constraints live under a "# Rules" heading; generic principle text is removed, never the domain content.
...
4. Score the file against "Audit criteria".
...
- `fact-checking`: still restates the shared principles above its domain rules. Removing those two lines; the domain rules stay byte-identical.
